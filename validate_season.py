...
for iseason in range(LAST_SEASON0 + 1):
    ...
    dcslen = len(bracket["DCS"])
    if dcslen != 7:
        raise Exception(
            f"Error: postseason DCS length is invalid: {dcslen} games, should be 7"
        )
    ...
    dcslen = len(postseason["DCS"])
    if dcslen > 7 or dcslen < 4:
        raise Exception(f"Error: postseason DCS length is invalid: {dcslen} games")
